...
public class BankingRegistrationController {
	...
	BankService bankServiceImpl;
	 ...
	 @Autowired
	 LamdaEmployeeService LamdaEmployeeServiceImpl;
	...
	//Creates a new bank
	   ...
	   // @ExceptionHandler(BankingBusinessException.class)
		...
		public ResponseEntity<Void> lamdaTest( ) {
			try {
			
				LamdaEmployeeServiceImpl.lamdaOperation();
			} catch (Exception e) {
				
				logger.error("lamda operation failed");
			} 
